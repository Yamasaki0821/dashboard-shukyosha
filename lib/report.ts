 ...
 *
 * ⚠ 当初の軸2は「取りこぼし率＝施行件数に対する紹介の割合」だったが、**追わないことにした**
 *   （2026-09-04 山崎さん判断：正確な葬儀施行件数がKintoneに無いため）。
 *   **代わりの推計値を作らない。**画面にも出さない。この判断を蒸し返さない。
 *
 * 守ること（霊園DBと同じ）：
...
function sectionHall(input: ReportInput): ReportSection {
  ...
  // 分母（施行件数）は追わないと決まっている。ここでは単価の差だけを見る
  ...
  if (items.length === 0) {
    items.push({
      key: 'hall-nodata',
      level: 'info',
      title: '会館別に語れるだけの件数がまだありません',
      body: [`${MIN_HALL_COUNT}件以上ある会館が3つ未満です。母数が小さいと1件あたりが暴れるため、比べていません。`],
      basis: '事業部・会館タブの会館別（件数）',
    });
  }
  ...
  return {
    key: 'hall',
    heading: '会館別',
    lead: '1件あたりの手数料の差を見ます。取りこぼし率（施行件数に対する紹介の割合）は追いません。',
    items,
  };
}
